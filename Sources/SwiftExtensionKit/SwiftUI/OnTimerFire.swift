...
@available(iOS 13.0, *)
public struct OnBackgroundTimerFireModifier: ViewModifier {
    let interval: TimeInterval
    let action: () -> Void
...
@available(iOS 13.0, *)
public extension View {
    func onBackgroundTimerFire(_ interval: TimeInterval, action: @escaping () -> Void) -> some View {
        self.modifier(OnBackgroundTimerFireModifier(interval: interval, action: action))
    }
}
